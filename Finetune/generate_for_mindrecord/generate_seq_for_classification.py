...
def convert_single_example(ex_index, example, label_list, max_seq_length,
    ...
    if data_format == 1:
        tokens_a=truncate_seq_pair_1x(tokens_a, max_seq_length - 3)
    else:
    ...
    if ex_index < 20:
        #temp_f = open("tensor.txt","w",encoding="utf-8")
        print("*** Example ***")
        print("guid: %s" % (example.guid))
        print("tokens: %s" % " ".join([tokenization.printable_text(x) for x in tokens]))
        print("input_ids: %s" % " ".join([str(x) for x in input_ids]))
        #input_ids_gh = " ".join([str(x) for x in input_ids])
        #temp_f.write(f"{input_ids_gh}\n")


        print("input_mask: %s" % " ".join([str(x) for x in input_mask]))
        # input_mask_gh =  " ".join([str(x) for x in input_mask])
        # temp_f.write(f"{input_mask_gh}\n")
        print("segment_ids: %s" % " ".join([str(x) for x in segment_ids]))
        # segment_ids_gh = " ".join([str(x) for x in segment_ids])
        # temp_f.write(f"{segment_ids_gh}\n")
