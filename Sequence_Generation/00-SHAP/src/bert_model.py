...
class BertSelfAttention(nn.Cell):
    ...
    def construct(self, input_tensor, attention_mask):
        attention_output,attention_probs = self.attention(input_tensor, input_tensor, attention_mask)
        output = self.output(attention_output, input_tensor)
...
class BertTransformer(nn.Cell):
    ...
    def construct(self, input_tensor, attention_mask):
        ...
        for layer_module in self.layers:
            layer_output,attention_probs = layer_module(prev_output, attention_mask)
            prev_output = layer_output
            prev_probs=attention_probs
            all_attention_probs = all_attention_probs + (prev_probs,)
...
class BertModel(nn.Cell):
    ...
    def construct(self, input_ids, token_type_ids, input_mask):
        ...
        # bert encoder
        encoder_output,attention_probs = self.bert_encoder(self.cast_compute_type(embedding_output),
                                           attention_mask)
        ...
        return sequence_output, pooled_output, embedding_tables,attention_probs
