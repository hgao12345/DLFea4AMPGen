...
class CrossEntropyCalculationV2(nn.Cell):
    """
    Cross Entropy loss
    """
    def __init__(self, is_training=True):
        super(CrossEntropyCalculationV2, self).__init__()
        self.onehot = P.OneHot()
        self.on_value = Tensor(1.0, mstype.float32)
        self.off_value = Tensor(0.0, mstype.float32)
        self.reduce_sum = P.ReduceSum()
    ...
    def construct(self, logits, label_ids,input_mask, num_labels):
        if self.is_training:
            label_ids = self.reshape(label_ids, self.last_idx)
            one_hot_labels = self.onehot(label_ids, num_labels, self.on_value, self.off_value)
            per_example_loss = self.neg(self.reduce_sum(one_hot_labels * logits, self.last_idx))
            attention_mask=self.reshape(input_mask, (-1,))
            assert F.shape(per_example_loss) == F.shape(attention_mask)
            per_example_loss = per_example_loss * attention_mask
            loss = self.reduce_mean(per_example_loss, self.last_idx) / self.reduce_mean(attention_mask)
            return_value = self.cast(loss, mstype.float32)
        else:
            return_value = logits * 1.0
        return return_value
